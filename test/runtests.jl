using Test
using Random
using MPO_MeanField
using ITensors, ITensorMPS
using LinearAlgebra

Random.seed!(0x5eed)

function dense_matrix(mpo, sys)
    N = 2^sys.params.L
    return [MatrixChecker(mpo, sys.sites, i, j, sys.bra_states, sys.ket_states) for i in 1:N, j in 1:N]
end

include("test_construction.jl")
include("test_public_configuration.jl")
include("test_public_solve.jl")
include("test_public_campaign.jl")
include("test_checkpoint.jl")
include("test_public_lside6_campaign.jl")
include("test_public_kpm.jl")
include("test_public_kpm_rectangular.jl")
include("test_public_kpm_graph.jl")
include("test_zero_safe.jl")
include("test_basis_conventions.jl")
include("test_hamiltonian_1d.jl")
include("test_hamiltonian_square.jl")
include("test_fields_1d.jl")
include("test_fields_square.jl")
include("test_energy_1d.jl")
include("test_energy_square.jl")
include("test_square_scf_block.jl")
include("test_purification_validation.jl")
include("test_scf_noninteracting.jl")
include("test_scf_interacting_dense_reference.jl")
include("test_purification_result.jl")
include("test_spectral_bounds.jl")
include("test_square_scf_bounds.jl")
include("test_scf_square_noninteracting.jl")
include("test_scf_square_interacting_dense_reference.jl")
include("test_scf_square_staggered_seed.jl")
include("test_square_fock_backends.jl")
include("test_scf_history.jl")
include("test_sp2.jl")
include("test_sp2_polynomials.jl")
include("test_purification_crossvalidation.jl")
include("test_sp2_difficult_cases.jl")
include("test_mcweeny_mu.jl")
include("test_mcweeny_horner.jl")
include("test_progress_output.jl")
include("test_cleanup_policy.jl")
include("test_end_to_end_regressions.jl")
include("test_kpm_local_helpers.jl")
include("test_qtt_kpm_mps_probe.jl")
include("test_qtt_charge_analysis.jl")

println("All MPO_MeanField tests completed successfully.")
